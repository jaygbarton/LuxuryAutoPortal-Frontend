...
function vehicleDisplayName(v: VehicleEntry): string {
  ...
  const nick =
    ...
    (v.device_nickname && v.device_nickname !== "[object Object]"
      ? v.device_nickname
      : null);
...
function vehicleSidebarName(v: VehicleEntry): string {
  ...
  const nick =
    ...
    (v.device_nickname && v.device_nickname !== "[object Object]"
      ...
      : null);
  const nameBase =
    make && model ? `${make} ${model}` : nick || `Device ${v.imei}`;
...
function getStatusInfo(status: string) {
  switch (status) {
    ...
    case "online":
      return {
        label: status === "parked" ? "Parked" : "Online",
        color: "#3b82f6",
        bg: "bg-blue-500",
      ...
      <div className="grid grid-cols-3 gap-px bg-gray-100 mx-4 rounded-lg overflow-hidden mb-3">
        {[
          { icon: Clock, value: lastSeen, label: "Last seen" },
          {
            icon: Activity,
            value:
              odometer != null ? `${Number(odometer).toLocaleString()}` : "—",
            label: "Miles",
          },
          {
            icon: fuelLevel != null ? Fuel : Battery,
            value:
              fuelLevel != null
                ? `${Math.round(fuelLevel)}%`
                : v.battery_level != null
                  ? `${Math.round(v.battery_level)}%`
                  : "—",
            label: fuelLevel != null ? "Fuel" : "Battery",
          },
        ].map((s, i) => (
...
function FleetMap({
  ...
  const layersRef = useRef<{ sat: any; labels: any; road: any }>({
    sat: null,
    labels: null,
    road: null,
  });
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      const labels = L.tileLayer(
        TILES.satelliteLabels.url,
        TILES.satelliteLabels.opts,
      );
      ...
      if (markerMap.current[v.device_id]) {
        ...
        if (moved) {
          slideMarkerTo(
            ...
            lat,
            lng,
            v.displayStatus === "driving" ? 2000 : 400,
          );
        }
  ...
  return (
    <div className="relative h-full">
        ...
        <div className="absolute inset-0 z-[800] flex flex-col items-center justify-center pointer-events-none">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl px-8 py-6 shadow-lg flex flex-col items-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Vehicles will appear once Bouncie reports their position
            </p>
          </div>
        </div>
      )}
      ...
      <button
        onClick={() =>
          setMapType((t) => (t === "satellite" ? "road" : "satellite"))
        }
        ...
        title={
          mapType === "satellite" ? "Switch to road map" : "Switch to satellite"
        }
      >
        {mapType === "satellite" ? (
...
export default function BouncieFleetPage() {
  ...
  useEffect(() => {
    ...
    if (connected === "true" || error) {
      if (connected === "true") {
        ...
        toast({
          title: "Connected!",
          description: "You're all set — your vehicles are now tracking live.",
        });
        ...
        const messages: Record<string, string> = {
          missing_code:
            "Looks like you cancelled. No worries, try again when you're ready.",
          invalid_state:
            "Something went wrong with the login. Please try again.",
          token_exchange_failed:
            "We couldn't finish connecting. Please try once more.",
        };
        toast({
          title: "Couldn't connect",
          description: messages[error] || `Something went wrong: ${error}`,
          variant: "destructive",
        });
      }
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const es = new EventSource(buildApiUrl("/api/bouncie/sse"), {
        ...
        try {
          ...
          if (payload?.type === "token_expired") {
            ...
            if (!tokenExpiredNotifiedRef.current) {
              ...
              toast({
                ...
                description:
                  "We couldn't renew your session. Please reconnect.",
                variant: "destructive",
              });
            }
            return;
          ...
          if (payload?.type === "token_refreshed") {
            ...
            toast({
              ...
              description: "Your session was renewed automatically.",
            });
            queryClient.invalidateQueries({
              queryKey: ["/api/bouncie/connection-status"],
            });
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Disconnected",
        description: "Bouncie tracking has been turned off.",
      });
    },
    onError: () => {
      toast({
        title: "Oops",
        description: "Couldn't disconnect right now. Give it another try.",
        variant: "destructive",
      });
    },
  });
            ...
            <div className="px-4 pb-2">
              ...
              {isError && !isLoading && (
                <p className="text-[11px] text-red-400 mt-1">
                  Having trouble loading vehicles.
                </p>
              )}
            </div>
          )}
                ...
                filtered.map((v) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-medium text-gray-100 truncate leading-tight">
                          {vehicleSidebarName(v)}
                        </div>
          ...
          {!connLoading && allVehicles.length === 0 && !isLoading && (
            <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
              ...
              <p className="text-[10px] text-gray-600 mt-3 leading-relaxed max-w-[200px]">
                You'll be asked to sign in to your Bouncie account. It only
                takes a moment.
              </p>
            </div>
          )}
          ...
          {(connLoading || isLoading) && allVehicles.length === 0 && (
            <div className="flex-1 flex items-center justify-center text-gray-500">
              ...
              <span className="text-sm">
                {connLoading ? "Checking connection…" : "Loading fleet…"}
              </span>
            </div>
          )}
